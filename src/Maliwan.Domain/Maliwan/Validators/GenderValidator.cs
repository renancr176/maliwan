...
public class GenderValidator : EntityValidator<Gender>, IGenderValidator
{
    ...
    private readonly IStringLocalizer<GenderValidator> _localizer;
    private readonly IGenderRepository _genderRepository;

    public GenderValidator(IStringLocalizer<GenderValidator> localizer, IGenderRepository genderRepository)
    {
    ...
    private async Task<bool> UniqueSkuAsync(Gender entity, string sku, CancellationToken arg3)
    {
        return !await _genderRepository.AnyAsync(
            e => e.Id != entity.Id && e.Sku.Trim().ToLower() == sku.Trim().ToLower());
    }

    private async Task<bool> UniqueNameAsync(Gender entity, string name, CancellationToken arg3)
    {
        return !await _genderRepository.AnyAsync(e =>
